...
def handle_response(command_name, command_data: CommandData):
    if command_data is None:
        print(f"Command: {command_name}, Didn't get the response, timeout")
        return
    
    if command_data.data_length == 1:
        result = command_data.data[0]
        if result == 1:
            print(f"Command: {command_name}, Received response success of: {hex(command_data.command_type)}, result: {result}")
        else:
            print(f"Command: {command_name}, Received response failed of: {hex(command_data.command_type)}, result: {result}")
    else:
        print(f"Command: {command_name}, Received unexpected length data: {command_data.to_dict()}")

if __name__ == "__main__":
    processor = CommandProcessor("command_data.json")
    serial_handler = SerialHandler(port="COM1", baudrate=115200, timeout=10)
    ...
    while True:
        ...
        try:
            ...
            print(f"Encoded command: {encoded_command.hex()}")
